      ...
      .then(async (values) => {
        ...
        try {
          ...
          const response = await axios.post(
            `${baseurl}/api/v1/review/review-section`,
            { userName: values.name, review: values.text }
          );
  ...
  const getReview = async () => {
    try {
      const response = await axios.get(
        `${baseurl}/api/v1/review/get-review`
      );
    ...
    <>
      <div className={reviewStyles.reviewContainer}>
        ...
        >
          ...
          {reviewData.map((data) => (
            <p>
              <hr /> <b>Name</b> : {data.userName} <br />
              <br /> <b>Review</b> : {data.review}
            </p>
          ))}
        </div>
      </div>
